...
- **HCP-mediated prevention / early-diagnosis recommendations for at-risk
  asymptomatic individuals** (e.g., first-degree relatives of cancer
  patients, individuals with chronic infections classified by IARC as
  Group 1 carcinogens, individuals with established occupational
  exposures). Output: a **Prevention Plan** parallel to the Treatment
  Plan, generated by the same engine, from the same KB, with the same
  HCP-targeted persona, surfaced through the same patient-mode translation
  layer (PATIENT_MODE_SPEC). **(Path A per
  `docs/plans/openonco_prevention_scope_2026-05-18-1500.md`, ratified
  under §6.1 dev-mode exemption 2026-05-18.)**
...
- **Patient-direct prevention service (Path B per the scope proposal).**
  Parked for v1.0+ — would trigger §15.3 re-classification as a medical
  device; requires Clinical Co-Lead expansion (§4.3), legal-structure
  formalization (§13), and a separate regulatory pathway.
- **Multi-cancer early-detection (MCED, e.g., ctDNA panels like Galleri)
  as a primary recommendation** — per §15 C5 (sources must be
  established/well-understood; MCED is still in trials). Reference-only
  mention is permitted when clinically relevant.
- **Image- or signal-based risk modeling** (e.g., mammography-AI risk
  scores) — per §15 C1/C3 (no raw image / signal input). Risk results
  extracted from a radiology *report* are accepted; raw pixels are not.
...
> **⚠ Temporary dev mode (from 2026-04-26, extended 2026-05-18):**
> Clinical Co-Leads have not yet been appointed, so the standard
> "consensus of three Co-Leads + Project Coordinator" gate is structurally
> impossible during the v0.1 phase. Following the precedent of §6.1 and
> §7.3 dev-mode exemptions, CHARTER amendments during v0.1 may be ratified
> by the Project Initiator alone under the following conditions:
> (a) the amendment is recorded in this CHANGELOG with explicit
> dev-mode-exemption mark and date; (b) the amendment does **not** weaken
> any §15 C1–C7 invariant (these are regulatory-bright-line constraints
> and remain inviolable in dev mode); (c) the amendment is queued for
> formal Co-Lead ratification once the first Co-Lead is appointed.
> Tracker: roadmap → "Operations → Reviewer signoff workflow". Synchronized
> with §6.1 / §7.3 dev-mode boxes.

- Changes to this document — by consensus of all three Clinical Co-Leads
  + Project Coordinator (suspended in dev mode per box above)
...
- **2026-05-18 — §3 scope amendment** (Initiator-only, dev-mode §6.1
  ...
  primary recommendation, and image/signal-based risk modeling as
  out-of-scope. §15 C1–C7 invariants unchanged. See
  `docs/plans/openonco_prevention_scope_2026-05-18-1500.md` for the
